...
//!
//! These form the typed foundation of the hello-tofu library:
...
impl TerraProvider {
    ...
    ///
    /// ```ignore
    /// assert_eq!(TerraProvider::AWS.local_name(), "aws");
    /// assert_eq!(TerraProvider::CLOUDFLARE.local_name(), "cloudflare");
    /// ```
    ...
    ///
    /// ```ignore
    /// assert_eq!(TerraProvider::AWS.short_source(), "hashicorp/aws");
    /// ```
...
mod tests {
    ...
    #[test]
    fn custom_provider() {
        let p = TerraProvider::new("My Provider", "registry.opentofu.org/acme/thing", "~> 1.0");
        assert_eq!(p.local_name(), "thing");
        assert_eq!(p.short_source(), "acme/thing");
    }
    ...
    fn resource_filter_restricts() {
        ...
        assert!(!f.is_empty());
        assert!(f.allows("registry.opentofu.org/hashicorp/aws", "aws_s3_bucket"));
        assert!(f.allows("registry.opentofu.org/hashicorp/aws", "aws_lambda_function"));
        assert!(!f.allows("registry.opentofu.org/hashicorp/aws", "aws_ec2_instance"));
        assert!(!f.allows(
    ...
    fn resource_filter_accumulates() {
        ...
        assert!(f.allows("p1", "r1"));
        assert!(f.allows("p1", "r2"));
        assert!(f.allows("p2", "r3"));
        assert!(!f.allows("p2", "r1"));
    }
}
