...
use hello_tofu::providers::aws_lightsail::*;
use serde_json::json;
...
"#,
    ...
    instance.key_pair_name = Some("${aws_lightsail_key_pair.keypair.name}".into());
    ...
    ports.port_info = Some(vec![
        AwsLightsailInstancePublicPortsResourceBlockTypePortInfo::new(8080, "tcp".into(), 8080),
        AwsLightsailInstancePublicPortsResourceBlockTypePortInfo::new(22, "tcp".into(), 22),
    ]);
    ...
    let out_dir = std::env::temp_dir().join("hello-tofu-lightsail");
    let out_path = out_dir.join("main.tf.json");
    let result = exporter.export_and_validate(&out_path)?;
    println!("tofu validate output: {}", result);
